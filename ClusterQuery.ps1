param (
    ...
    [boolean]$TestWinRMAccess= $true   #by default tests WinRM network access
    )
    ...
    $script =
        ...
        #Get info about ODBC DSNs
            ...
            foreach ($source in $ODBCsources) {
                    ...
                    $result | Add-Member -Name "ODBCSource$($i)_Name" -MemberType NoteProperty -Value "$($source.Name)"

                    $result | Add-Member -Name "ODBCSource$($i)_DriverName" -MemberType NoteProperty -Value "$($source.DriverName)"
